...
export async function previewChange(opts:{ planId:string; modules:{id:string;opt?:OptKey}[]; cycle:BillingCycle }, ctx: HttpContext){
     ...
     if (planCur === planNext) {
          ...
          const isUpgrade = (debitBy[planNext] ?? 0) > 0

          if (isUpgrade) {
               // ✅ UPGRADE → nouveau plan actif immédiatement
               planChange = {
                    id: ctx.subscription.current_plan_id,
                    name: currentPlanRow.name?.[userLang] ?? currentPlanRow.name?.['en'] ?? 'Plan',
                    effective_date: endOfPeriodDate, // <- facultatif, historique
                    effective_action: 'replaced_immediately',
                    price_now: 0,
