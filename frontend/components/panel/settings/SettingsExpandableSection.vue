<template>
     <div class="relative rounded-[12px] border border-[rgb(226,232,239)] mb-[20px]">
          ...
          <button
               ...
               >{{ title }} <div v-if="isBilled" :class="isPaid ? 'bg-[rgb(190,255,203)]' : 'bg-[rgb(255,219,186)]'" class="p-[3px_5px] rounded-[4px] block ml-[8px]" > <div class="flex flex-row justify-start items-center"> <svgo-chat-icon-premium class="w-[14px] h-[14px] block fill-[rgb(8,15,26)] transition-none mr-0" /> <p class="mt-0 mb-0 ml-[2px] font-normal text-[12px] leading-[16px] tracking-[-0.01em]"> {{ isPaid ? 'Payé' : 'Payant' }} </p> </div> </div></h3> <svgo-chat-icon-errgel class="w-[24px] h-[24px] ml-auto rotate-0 transition-transform duration-300 ease-in-out mr-[20px] mt-[20px]" :class="isOpen ? 'rotate-0' : 'rotate-180'" />
          </button>
